...
$lss = $conn->query("SELECT * FROM servicos  
                     where dataservicos >= CURDATE() 
                     ORDER BY dataservicos ASC 
...
foreach ($lss as $serv) {
  ...
  $totalEscalados = $conn->query("SELECT count(idmilitar1) as totalescalado 
                                  FROM servicosescala 
                                  WHERE idservicos = $serv->idservicos")
    ->fetch(PDO::FETCH_OBJ);


  $totalVoluntarios = $conn->query("SELECT count(idmilitar) as totalvoluntario 
                                    FROM servicosvoluntario 
                                    WHERE idservicos = $serv->idservicos")
    ->fetch(PDO::FETCH_OBJ);

  $esc = $conn->query(" SELECT usuarios.cargo, usuarios.nomeguerra, servicosescala.idservicos, servicosescala.idmilitar1 
                        FROM usuarios, servicosescala 
                        WHERE usuarios.codfunc = servicosescala.idmilitar1 
                        AND servicosescala.idservicos = $serv->idservicos ")
    ->fetchAll(PDO::FETCH_OBJ);
